...
namespace xt
{
    ...
    TEST(xsort, argmax_prob)
    {
        for (std::size_t i = 0; i < 20; ++i)
        {
            ...
            auto a_s0 = argmin<layout_type::column_major>(a, 0);
            auto va_s0 = view(a, xt::all(), 3, 2, 3);
            auto m0_idx = a_s0(3, 2, 3);
            auto it0 = std::min_element(va_s0.template begin<layout_type::column_major>(),
                                        va_s0.template end<layout_type::column_major>());
            auto c0_idx = std::distance(va_s0.template begin<layout_type::column_major>(),
                                        it0);
            EXPECT_EQ(static_cast<size_t>(c0_idx), m0_idx);

            auto a_s1 = argmin<layout_type::row_major>(a, 1);
            auto va_s1 = view(a, 3, xt::all(), 2, 3);
            auto m1_idx = a_s1(3, 2, 3);
            auto it1 = std::min_element(va_s1.template begin<layout_type::row_major>(),
                                        va_s1.template end<layout_type::row_major>());
            auto c1_idx = std::distance(va_s1.template begin<layout_type::row_major>(),
                                        it1);
            EXPECT_EQ(static_cast<size_t>(c1_idx), m1_idx);

            auto a_s2 = argmin<layout_type::column_major>(a, 2);
            auto va_s2 = view(a, 3, 2, xt::all(), 3);
            auto m2_idx = a_s2(3, 2, 3);
            auto it2 = std::min_element(va_s2.template begin<layout_type::column_major>(),
                                        va_s2.template end<layout_type::column_major>());
            auto c2_idx = std::distance(va_s2.template begin<layout_type::column_major>(),
                                        it2);
            EXPECT_EQ(static_cast<size_t>(c2_idx), m2_idx);

            auto a_s3 = argmin<layout_type::row_major>(a, 3);
            auto va_s3 = view(a, 3, 2, 3, xt::all());
            auto m3_idx = a_s3(3, 2, 3);
            auto it3 = std::min_element(va_s3.template begin<layout_type::row_major>(),
                                        va_s3.template end<layout_type::row_major>());
            auto c3_idx = std::distance(va_s3.template begin<layout_type::row_major>(),
                                        it3);
