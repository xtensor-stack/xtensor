...
#include "xtensor/xnd_iterator.hpp"

namespace xt
{
    using std::size_t;

    xarray<int> get_nd_test_array()
    {
    ...
    TEST(xnd_iterator, begin)
    {
        xarray<int> a = get_nd_test_array();
        auto iter_begin = xnd_axis_begin(a, 0);
    ...
    TEST(xnd_iterator, end)
    {
        xarray<int, layout_type::column_major> a = get_nd_test_array();
        auto iter_begin = xnd_axis_begin(a);
        auto iter_end = xnd_axis_end(a);
        ++iter_begin; ++iter_begin;
        EXPECT_EQ(iter_begin, iter_end);
        xarray<int> b = get_nd_test_array();
        auto iter_begin_row = xnd_axis_begin(b, 2);
        auto iter_end_row = xnd_axis_end(b, 2);
        ++iter_begin_row; ++iter_begin_row; ++iter_begin_row; ++iter_begin_row;
        EXPECT_EQ(iter_begin_row, iter_end_row);
    }

    TEST(xnd_iterator, increment)
    {
        xarray<int> a = get_nd_test_array();
        auto iter = xnd_axis_begin(a, 0);
        ++iter;
    ...
    TEST(xnd_iterator, nested)
    {
        xarray<int> a = get_nd_test_array();
        auto iter = xnd_axis_begin(a, 0);
        ++iter;
        auto niter = xnd_axis_begin(*iter, 0);
        ++niter;
    ...
    TEST(xnd_iterator, const_array)
    {
        const xarray<int> a = get_nd_test_array();
        auto iter = xnd_axis_begin(a, 0);
        ++iter;
    ...
    TEST(xnd_iterator, axis_0)
    {
        xarray<int> a = get_nd_test_array();
        auto iter = xnd_axis_begin(a, size_t(0));
    ...
    TEST(xnd_iterator, axis_1)
    {
        xarray<int> a = get_nd_test_array();
        auto iter = xnd_axis_begin(a, size_t(1));
    ...
    TEST(xnd_iterator, axis_2)
    {
        xarray<int> a = get_nd_test_array();
        auto iter = xnd_axis_begin(a, size_t(2));
