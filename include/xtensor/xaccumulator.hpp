...
#define DEFAULT_STRATEGY_ACCUMULATORS evaluation_strategy::immediate_type
    ...
    auto make_xaccumulator_functor(RF&& accumulate_func, IF&& init_func)
    {
        using accumulator_type
            = xaccumulator_functor<std::remove_reference_t<RF>, std::remove_reference_t<IF>>;
    ...
    namespace detail
    {
        ...
        {
            static_assert(!std::is_same<evaluation_strategy::lazy_type, EVS>::value,
        ...
        template <class T, class R>
        using xaccumulator_linear_return_type_t =
        ...
        inline auto accumulator_init_with_f(F&& f, E& e, std::size_t axis)
        {
            ...
            auto set_loop_sizes
                = [&outer_loop_size, &inner_loop_size](auto first, auto last, std::ptrdiff_t ax) {
                      outer_loop_size = std::accumulate(
                          first, first + ax, std::size_t(1), std::multiplies<std::size_t>());
                      inner_loop_size = std::accumulate(
                          first + ax + 1, last, std::size_t(1), std::multiplies<std::size_t>());
                  };

            // Note: add check that strides > 0
            auto set_loop_strides = [&outer_stride,
                                     &inner_stride](auto first, auto last, std::ptrdiff_t ax) {
                outer_stride
                    = static_cast<std::size_t>(ax == 0 ? 1 : *std::min_element(first, first + ax));
                inner_stride
                    = static_cast<std::size_t>((ax == std::distance(first, last) - 1)
                                                   ? 1
                                                   : *std::min_element(first + ax + 1, last));
            };

            set_loop_sizes(e.shape().begin(), e.shape().end(), static_cast<std::ptrdiff_t>(axis));
            set_loop_strides(
                e.strides().begin(), e.strides().end(), static_cast<std::ptrdiff_t>(axis));
        ...
        template <class F, class E>
        inline auto accumulator_impl(F&& f,
                                     E&& e,
                                     std::size_t axis,
                                     evaluation_strategy::immediate_type)
        {
            ...
            std::size_t outer_stride
                = 1;  // this is either going row- or column-wise (strides.back / strides.front)
            std::size_t outer_loop_size = 0;
            std::size_t inner_loop_size = 0;
            std::size_t init_size
                = e.shape()[axis] != std::size_t(1) ? std::size_t(1) : std::size_t(0);

            auto set_loop_sizes = [&outer_loop_size, &inner_loop_size, init_size](
                                      auto first, auto last, std::ptrdiff_t ax) {
                outer_loop_size
                    = std::accumulate(first, first + ax, init_size, std::multiplies<std::size_t>());

                inner_loop_size = std::accumulate(
                    first + ax, last, std::size_t(1), std::multiplies<std::size_t>());
            };

            if (result_type::static_layout == layout_type::row_major)
            {
                set_loop_sizes(result.shape().cbegin(),
                               result.shape().cend(),
    ...
    {
        // Note we need to check is_integral above in order to prohibit EVS = int, and not taking
        // the std::size_t overload below!
        return detail::accumulator_impl(
            std::forward<F>(f), std::forward<E>(e), evaluation_strategy);
    }
    ...
    inline auto accumulate(F&& f, E&& e, std::ptrdiff_t axis, EVS evaluation_strategy = EVS())
    {
        ...
        return detail::accumulator_impl(
            std::forward<F>(f), std::forward<E>(e), ax, evaluation_strategy);
    }
}
