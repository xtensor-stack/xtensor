...
#define DEFAULT_STRATEGY_ACCUMULATORS evaluation_strategy::_immediate
    ...
    namespace detail
    {
        ...
        {
            static_assert(!std::is_same<evaluation_strategy::_lazy, EVS>::value, "Lazy accumulators not yet implemented.");
        }
        ...
        template <class F, class E>
        inline auto accumulator_impl(F&& f, E&& e, std::size_t axis, evaluation_strategy::_immediate)
        {
        ...
        template <class F, class E>
        inline auto accumulator_impl(F&& f, E&& e, evaluation_strategy::_immediate)
        {
            using accumulate_functor = std::decay_t<decltype(std::get<0>(f))>;
