...
namespace xt
{
    namespace detail
    {
        ...
        struct provides_data_interface
            : xtl::conjunction<has_data_interface<std::decay_t<E>>,
    ...
    public:
        ...
        static constexpr bool is_const
            = std::is_const<std::remove_reference_t<undecay_expression>>::value;
        ...
        static constexpr bool contiguous_layout
            = static_layout != layout_type::dynamic && xexpression_type::contiguous_layout;

        template <class CTA, class SA>
        xstrided_view_base(CTA&& e,
                           SA&& shape,
        ...
        template <class E = xexpression_type, class ST = storage_type>
        std::enable_if_t<detail::provides_data_interface<E, ST>::value, const_pointer> data() const
            noexcept;
    ...
    inline xstrided_view_base<D>::xstrided_view_base(
        CTA&& e, SA&& shape, strides_type&& strides, size_type offset, layout_type layout) noexcept
    ...
    {
        return static_cast<offset_type>(m_offset)
               + xt::unchecked_data_offset<offset_type>(strides(),
                                                        static_cast<offset_type>(args)...);
    }
    ...
    {
        return static_cast<offset_type>(m_offset)
               + xt::element_offset<offset_type>(strides(), first, last);
    }
    ...
    namespace detail
    {
        ...
        template <class CT, layout_type L>
        inline auto flat_expression_adaptor<CT, L>::operator[](size_type idx) const
            -> const_reference
        {
            auto i = static_cast<typename index_type::value_type>(idx);
            m_index = detail::unravel_noexcept(i, m_strides, L);
        ...
        struct strided_view_args : adj_strides_policy
        {
                           ...
                           const V& slices)
            {
                ...
                for (; i < slices.size(); ++i)
                {
                    ...
                    else if (base_type::fill_args(slices,
                                                  i,
                                                  idx,
                                                  old_shape[i_ax],
                                                  old_strides[i_ax],
                ...
                new_layout = do_strides_match(new_shape, new_strides, layout, true)
                                 ? layout
                                 : layout_type::dynamic;
            }
