...
namespace xt
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            // using iterator = typename xexpression_type::layout_iterator<XTENSOR_DEFAULT_LAYOUT>;
            // using const_iterator = typename xexpression_type::const_layout_iterator<XTENSOR_DEFAULT_LAYOUT>;

            using iterator = decltype(std::declval<std::decay_t<CT>>().template begin<XTENSOR_DEFAULT_LAYOUT>());
            using const_iterator = decltype(std::declval<std::decay_t<const CT>>().template begin<XTENSOR_DEFAULT_LAYOUT>());
