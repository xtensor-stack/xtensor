...
namespace xt
{
    ...
    E1& chunked_assign_expression(E1&& e1, E2&& e2)
    {
        std::vector<std::size_t> chunk_shape;
        chunk_shape.resize(e1.dimension());
        const auto& s = e1.chunk_shape();
        std::copy(s.cbegin(), s.cend(), chunk_shape.begin());
        auto chunked_view = xchunked_view<E2>(std::forward<E2>(e2), chunk_shape);
        auto& chunks = e1.chunks();
        auto chunk_it = chunks.begin();
        for (auto it = chunked_view.begin(); it != chunked_view.end(); it++)
        {
            auto sv = *it;
            auto rhs = strided_view(e2, sv);
            auto rhs_shape = rhs.shape();
            if (rhs_shape != e1.chunk_shape())
            {
